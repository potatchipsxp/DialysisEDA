...
# Introduction #
  Dialysis is the practice of mechanically cleaning blood for someone, usually someone whose kidney are not working well enough to clean it themselves. This field is growing rapidly and is attracting a significant amount of public attention such as the recent proposition 22. As someone who is married to a dialysis social worker my day is constantly filled with stories about about how procedures operate in this field. In particular I hear a great deal of stories about how patient characteristics ANECDOTALLY seem to affect their longterm outcome, and how patient characteristics seem to affect the efficacy of the center. I decided to do an EDA on Dialysis center data and search for ways to quantify these relationships such that dialysis centers could make more informed decisions about how to help patients, and how to evaluate their efforts. To do this I downloaded dialysis center data from CMS.gov who evaluates dialysis facilities around the country. I then gathered regional economic data at the county level, as I am not privvy to individual patient data. (more details about the data, as well as the URLs are in the readme)
...
# EDA #
  ...
  Lets begin with our medical data. This data actually comes from two different data sets. The furst data set is survey data in which patients are handed out surveys and respond to questionnaire items such as how well they feel taken care of at their facility, how much they trust the staff, etc. The second is from mandated state reviews of dialysis facilities in which the state comes and reviews the facility every X amount time. First lets look at the survey data. 
...
the survey data has `r nrow(dataS)` rows which would correspond to the averaged responses for every individual facility in the country and `r ncol(dataS)` columns which correspond to the variables being measured by the questionnaire such as nephrologist bedside manner facility quality of care and operations. 

The question we are interested in here is how well state review of the medical practice in the facility corresponds to patient reviews of the facility. To do that we will need to also look at state review data which has `r nrow(dataR)` rows which again corresponds to all every dialysis facility and `r ncol(dataR)` columns which correspond to the variables measured in the state review such as type of access in patients, quality of facility operations, amount of patients with high kt over V and other variables important to describing the medical care provided by the facility.

We combined these two data sets into a single set for our analysis. We also cleaned the data and removed bad rows and redundant columns. For example, because this data set contains every state approved dialysis facility many of the dialysis facilities in the dataset are small and didnt have adequate data. In addition many of the variables measured only apply to certain facilities such as number of pediatric patients which only applies to dialysis center that have pediatric care. We removed any columns, like these, which were providing values of null for more than half of the data set We also dropped all non numeric variables as this is a simple exploratory analysis, though future analyses might take those into account. At this point we were left with `r nrow(dataM_N)` facilities and `r ncol(dataM_N)` variables left in our medical data. 

one interesting first pass analysis is to look at the difference between for profit and not for profit dialysis centers. 
...
```
Here we see that nonprofit dialysis centers are actually significantly better rated by their patients. Although, the practical difference does not seem to be very high.


As we are interested in how medical reviews relate to patient facility review we created a correlation matrix and then selected the top 10 correlation with facility review (a whole correlation matrix was way too big). 
...
Several interesting things pop out here. The first is that while patient's feelings about the staff are very related to their review of the facility, their doctor is very unrelated. I followed this up with an interview with a social worker who works at a local dialysis center (my wife) and she informed that one of the biggest frustration patients have is the dialysis doctors bedside manner which is often rushed and impersonal. This is compounded by the fact that there are very few nepophrologists. In the  Merced area there are only a handful of nephrologists, but hundreds if not thousands of regular dialysis patients. 

A second interesting thing to observe in this correlation matrix is that one of the only medical items that makes it into the top ten correlations with facility review is the rate of A/V fistula accesses. AV fistula the most recommended type of access point because it is least prone to infection. In fact according to the same social worker, the dialysis facilities in her company are evaluated based on the percentage of patients they have who have AV fistula access points. However, the AV fistula is not installed first on patients who "crash" into dialysis because it is not as easy as some other access points to install and is therefore not installed in emergency situations. Many patients however, especially in low income areas where medical care is less available, are not aware of their kidney problems until they are hospitalized and have to have emergency dialysis. In this sense it might be of interest to companies who evaluate their dialysis center based on the percentage of Av fistula access points to know how much to account for income in the area surrounding the facility during evaluations.

To do this we need examine the relationship between economic data in the surrounding area on the rate of av fistula for each facility. The interviewee also posited that the education level of patients makes a difference in how easy it is to convince the patient to get the av fistula because many patients simply refuse to switch despite the benefits. We curated a dataset of economic and education data at the county level in the united states. We then combined that data with a subset of our Medical data and made a correlation matrix to investigate.
...
```
here it would seem that median household income is a significant and useful predictor of AV fistula rate with a standardized beta of .2012   and p<.001. This is not a huge R squared, but as we discuss later, the economic data we are using is at the county level and there is a significant amount of variance in economic data for patients at dialysis facilities that is neglected by looking only at the level of county. Infact I would postualte that finding an effect for county is rather remarkable given the amount of variance that is aggregated over in individual incomes within a county.  
